  ...
  <AppLayout>
    ...
    <div class="mb-6">
      ...
      <h1 class="text-2xl font-semibold text-slate-900 dark:text-slate-100">
        Edit Purchase Order
        <span class="font-mono text-indigo-600 dark:text-primary-400 ml-2">{{ order.po_number }}</span>
      </h1>
    </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input v-model.number="form.exchange_rate" type="number" step="0.000001" min="0"
                class="w-full pl-20 pr-12 rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:focus:ring-primary-900/30 px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 font-mono"
...
import {
  ...
  LoaderIcon, SearchIcon, CheckIcon, WeightIcon, ArrowLeftIcon,
...
function mapItem(existing) {
  ...
  return {
    ...
    supplier_price_cny: parseFloat(existing.supplier_price_cny),
    quantity:           existing.quantity,
    approx_weight_kg:   existing.approx_weight_kg ? parseFloat(existing.approx_weight_kg) : 0,
    // UI-only state
    product_search:  variantName ? `${productName} — ${variantName}` : productName,
    searchResults:   [],
    searching:       false,
    showDropdown:    false,
    _searchTimer:    null,
...
const form = reactive({
  ...
  currency_id:            props.order.currency_id,
  exchange_rate:          parseFloat(props.order.exchange_rate),
  order_date:             props.order.order_date?.split('T')[0] ?? props.order.order_date,
...
async function submit() {
  ...
  try {
    await window.axios.put(`/api/v1/purchase-orders/${props.order.id}`, payload)
    success('Purchase Order updated successfully!')
    router.visit(route('purchase-orders.show', props.order.id))
  } catch (err) {
    if (err.response?.status === 422) {
      ...
      showError(err.response?.data?.message || 'Failed to update purchase order.')
    }
    saving.value = false
  }
}
