<template>
  <AppLayout>
    ...
    <div class="flex items-start gap-3 mb-6">
      ...
      <div>
        <h1 class="text-2xl font-semibold text-slate-900 dark:text-slate-100">Edit {{ invoice.invoice_no }}</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Update invoice details, items, and adjustments.</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-5">
        ...
        <!-- SO Info (read-only) -->
        <div v-if="invoice.sales_order" class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/50 rounded-xl p-4">
          <h3 class="text-sm font-semibold text-blue-700 dark:text-blue-300 mb-2">Linked Sales Order</h3>
          <div class="grid grid-cols-2 gap-2 text-sm">
            <div>
              <span class="text-xs text-blue-500">Order No</span>
              <div class="font-mono font-semibold text-blue-800 dark:text-blue-200">{{ invoice.sales_order.order_no }}</div>
            </div>
            <div>
              <span class="text-xs text-blue-500">Customer locked</span>
              <div class="text-blue-700 dark:text-blue-300 text-xs">Customer is inherited from the SO and cannot be changed.</div>
            </div>
          </div>
        </div>

        <!-- Customer (standalone only) -->
        <div v-if="!invoice.sales_order_id" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
          <div class="flex items-center justify-between mb-3">
            ...
            <button
              type="button"
              @click="addItem"
              class="inline-flex items-center gap-1.5 text-xs text-primary-600 dark:text-primary-400 hover:text-primary-700 font-medium"
            >
            ...
            >
              <div class="col-span-12 sm:col-span-5">
                ...
                <input
                  ...
                  type="text"
                  class="w-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none"
                  readonly
                />
              </div>
      ...
      <!-- Right: Adjustments + Totals + Buttons -->
      <div class="space-y-5">
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
        ...
        <Link
          :href="route('invoices.show', invoice.id)"
          class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-xl transition-colors"
        >
          Cancel
        </Link>
      </div>
...
// ─── Pre-populate form from invoice ──────────────────────────────────────

const form = reactive({
  customer_id:         props.invoice.customer_id,
  issue_date:          String(props.invoice.issue_date).split('T')[0],
  due_date:            props.invoice.due_date ? String(props.invoice.due_date).split('T')[0] : '',
  notes:               props.invoice.notes ?? '',
  discount_bdt:        Number(props.invoice.discount_bdt ?? 0),
  delivery_charge_bdt: Number(props.invoice.delivery_charge_bdt ?? 0),
...
async function submit() {
  ...
  try {
    ...
    await window.axios.put(`/api/v1/invoices/${props.invoice.id}`, payload)
