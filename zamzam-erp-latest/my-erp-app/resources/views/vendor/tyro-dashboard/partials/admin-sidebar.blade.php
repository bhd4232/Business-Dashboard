<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav sidebar-accordion"
        ...
        @php
            $websiteResourceKeys = ['site_settings', 'site_banners', 'site_sections', 'site_pages', 'site_menus', 'contact_messages'];
            $websiteResourceModels = [
                'site_settings' => \App\Models\SiteSetting::class,
                'site_banners' => \App\Models\SiteBanner::class,
                'site_sections' => \App\Models\SiteSection::class,
                'site_pages' => \App\Models\SitePage::class,
                'site_menus' => \App\Models\SiteMenu::class,
                'contact_messages' => \App\Models\ContactMessage::class,
            ];
            $websiteResources = collect($allResources ?? config('tyro-dashboard.resources', []))->only($websiteResourceKeys);
            foreach ($websiteResourceModels as $resourceKey => $modelClass) {
                if (! $websiteResources->has($resourceKey) && class_exists($modelClass) && method_exists($modelClass, 'getResourceConfig')) {
                    $websiteResources->put($resourceKey, $modelClass::getResourceConfig());
                }
            }
        @endphp

        @if($websiteResources->isNotEmpty())
        <div class="sidebar-section">
            <div class="sidebar-section-title">Website</div>
            @foreach($websiteResources as $key => $resource)
                @php
                    $canAccessWebsiteResource = true;
                    if (isset($resource['roles']) && !empty($resource['roles'])) {
                        $canAccessWebsiteResource = false;
                        $user = auth()->user();
                        if ($user && method_exists($user, 'tyroRoleSlugs')) {
                            ...
                            if (!$canAccessWebsiteResource && isset($resource['readonly']) && !empty($resource['readonly'])) {
                ...
                @if($canAccessWebsiteResource)
                <a href="{{ route($dashboardRoute::name('resources.index'), $key) }}" class="sidebar-link {{ request()->is('*resources/'.$key.'*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 6.75A2.75 2.75 0 015.75 4h12.5A2.75 2.75 0 0121 6.75v10.5A2.75 2.75 0 0118.25 20H5.75A2.75 2.75 0 013 17.25V6.75z" />
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 9h18M8 14h5M8 16.5h8" />
                    </svg>
                    {{ $resource['title'] }}
                </a>
                @endif
        ...
        <div class="sidebar-section">
            ...
            @foreach($allResources ?? config('tyro-dashboard.resources', []) as $key => $resource)
                @php
                    if (in_array($key, $websiteResourceKeys ?? [], true)) {
                        continue;
                    }
