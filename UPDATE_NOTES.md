...
## 2026-09-17 - Reseller commission engine, paid through the shared payout batches (plan 10, Phase 2)

Reason — owner: after Phase 1 (Investor payout batches, entry below) was reviewed, asked to "পরবর্তি ফেজের কাজ কর" (do the next phase's work) — the Reseller commission side of plan 10, deferred at Phase 1's scope decision. Two rounds of clarifying questions were asked before coding, since real gaps turned up that CLAUDE.md's rule against inventing business rules wouldn't let me guess past:

1. First round — checked what "next phase" meant given the codebase had **no wholesale-price concept at all** (`ResellerProduct` was pure catalog-pick/visibility, `PROJECT_GUIDE.md` already noted reseller pricing was "intentionally deferred"). Owner answers: wholesale rate lives per reseller-per-product, staff-set (new field); the reseller's own selling price would be a **new self-service feature** (reseller sets it per product); "other costs" = packaging, return, and courier/delivery charge (an "other, please specify" option was selected but left blank).
2. Second round — with the reseller-selling-price question now clearly a full storefront-pricing change (checkout, offers/combos, WooCommerce sync all touch it), asked whether to build that too or keep this round to the commission/payout engine using the order's existing sale price. Owner: "শুধু Commission + Payout এন্জিন — বর্তমান order price দিয়েই" (just the commission/payout engine, using the current order price) — so commission is calculated off the order's actual sale price (today, the same public/tier price every customer sees), not a reseller-set retail price; that storefront feature is a deferred follow-up, not part of this round.

What was built:

- **4 new migrations**: `wholesale_rate` on `reseller_products` (nullable, staff-set); `reseller_payout_method`/`reseller_payout_details` (encrypted) on `customers`; `reseller_commissions` table (polymorphic-`payable`-compatible with the Phase 1 `PayoutBatch`/`PayoutItem` engine); `reseller_commission_hold_days` on `company_payout_settings` (default 3, per the owner's answer) and `recipient_mfs_number` on `payout_items`.
- **New model**: `ResellerCommission` (`hasActivePayoutItem()`/`hasCompletePayoutDetails()`, mirroring `SettlementPayout`'s Phase 1 helpers). `ResellerProduct` and `Customer::resellerCatalog()`'s pivot gained `wholesale_rate`; `Customer` gained the payout fields and a `resellerCommissions()` relation.
- **`App\Services\Reseller\ResellerCommissionService`** — `createForDeliveredOrder()` (sums `(unit_price − wholesale_rate) × quantity` per `OrderItem`, skipping items with no wholesale rate set rather than guessing one; starts `holding` with `holding_until` = today + the company's hold-day setting), `updateCostBreakdown()` (a generic `[{label, amount}]` list — packaging/return/courier are the owner's named categories, but nothing is hardcoded to just those three so a future one doesn't need a schema change), `promoteDueToPayable()` (per-company), `reverseForOrder()` (reverses only while still `holding`/`payable`; a commission already `in_batch`/`paid` is left untouched and the return is written to the audit log for manual follow-up instead — clawing back money already sent needs an owner-confirmed recovery rule the plan doesn't have yet).
- **`App\Observers\ResellerCommissionObserver`** (registered on `Order`, alongside `OrderNotificationObserver`) — creates the commission when `status`+`delivery_status` reach the same "delivered" pair `OrderStatusWorkflowService::transition(..., Order::STAGE_DELIVERED)` always sets together (covers manual staff action, courier webhooks, and Steadfast sync alike, since they all converge on that one transition call); reverses it on `returned`/`refunded`.
- **`reseller-commissions:promote`** scheduled command (daily 01:00, loops companies) calling `promoteDueToPayable()`.
- **`PayoutBatchService::createBatchFromPendingResellerCommissions(company, method, user)`** — same eligibility-filter-then-draft-batch shape as the Phase 1 Investor call, but for `ResellerCommission` rows whose reseller chose that exact method; kept as a separate call rather than merging with the Investor one (an Investor batch and a Reseller batch due the same day are two separate batches this round — documented as a possible later simplification, not a defect). Added `PayoutFormatterResolver` (picks `GenericBeftnCsvFormatter` for `beftn_bank`, a new `GenericMfsListFormatter` — plain name/MFS-number/amount CSV, no provider API yet — for every `mfs_*` method) and a `markPayablePaid()` helper in `reconcileItem()` so reconciling now correctly updates either payable type.
- **Filament**: Reseller edit form gained a "Commission Payout" section (method + bank/MFS details, encrypted); **Store Products** tab gained a staff-only "Set Wholesale Rate" action (the one deliberate exception to that tab's existing "reseller self-service only" read-only design, since wholesale rate is the company's own cost-basis decision, not the reseller's); new **Commissions** tab with an "Adjust Costs" action (only while `holding`/`payable`). **Payout Batches → Generate New Batch** gained a Source (Investor/Reseller) and, for Reseller, a Method selector. No new permission — gated by the same `Customer`-update permission (`canPerformModelAbility('update', Customer::class)`) the Reseller resource itself already uses for editing.
- **`10_INVESTOR_RESELLER_AUTO_PAYOUT_PLAN.md`** — added the Phase 2 build summary, updated the "Owner Decision লাগবে" checklist with both rounds' answers, and recorded the deferred storefront-pricing follow-up as its own explicitly-non-blocking item.

Verification:

- New tests: `tests/Feature/ResellerCommissionTest.php` (10 cases — calculation, no-commission for a non-reseller order, redelivery doesn't duplicate, items without a wholesale rate contribute nothing, promote timing before/after the hold period, reversal within the window, a paid commission is untouched by a late return, cost-breakdown math, and the reseller edit/view pages render the new sections) and `tests/Feature/ResellerCommissionPayoutBatchTest.php` (4 cases — method/complete-details filtering, the full batch lifecycle marking the commission paid, and the MFS formatter/recipient number). `MultiCompanyIsolationTest` updated with `ResellerCommission`.
- `php artisan test tests/Feature/ResellerCommissionTest.php tests/Feature/ResellerCommissionPayoutBatchTest.php tests/Feature/PayoutBatchTest.php tests/Feature/PayoutBatchPagesSmokeTest.php tests/Feature/InvestmentSettlementTest.php tests/Feature/MultiCompanyIsolationTest.php tests/Feature/ResellerResourceTest.php tests/Feature/ResellerSelfServiceTest.php tests/Feature/ResellerStorefrontTest.php tests/Feature/StorefrontResellerAndAbandonedCartTest.php tests/Feature/OrderStatusWorkflowTest.php` — 64 passed.
- Full `php artisan test` — **1211 passed, 1 failed**. The one failure (`ReleaseNotesTest > database related release notes are hidden from a non super admin viewer`) is the same pre-existing, unrelated failure already noted in the Phase 1 entry below — `git diff` against this session's starting `CHANGELOG.md` still shows no change to any already-published version section.
- `vendor/bin/pint --dirty` — clean (auto-fixed import ordering/unused imports/fully-qualified doc types along the way, all cosmetic).

Commit status: Committed + pushed to `origin/claude/auto-profit-payment-system-9acb0u` together with the Phase 1 entry below, in one commit (owner approved: "কমিট এবং পুশ কর, রিলিজ ভার্শন লেটেস্ট github ripo চেক করে লিখবে। এরপর কমিট এবং পুশ করবে শুধু তোমার এই সেশনের কাজগুলোই।"). Latest published release checked against the GitHub repo before writing this: no tags/releases exist there beyond what `CHANGELOG.md` already tracks (`2.15.0`), so this stays under `[Unreleased]` — the version bump is cut separately by the repo's own `chore(release)` automation, not by this commit.

## 2026-09-17 - Investor/Channel-Partner BEFTN payout batches (plan 10, Phase 1)

Reason — owner: after reviewing the plan document (see the 2026-09-11 entry below) and getting a printable bank-requirement brief to take to their bank, the owner asked for a checklist of what to bring to the bank, then said "চেকলিস্ট দেও কি কি লাগবে তারপর আমি ব্যাংকের সাথে কথা বলব" and, once that was answered, "ব্যাংকের সাথে কথা বলে এসে জানাচ্ছি, এখন পরবর্তি কাজ এ এগিয়ে যায়" (will report back after talking to the bank; proceed with the next work meanwhile). Clarifying questions were asked first (answers below) since coding could start on the bank-independent half without waiting on the bank.

Owner answers (asked via AskUserQuestion before writing any code): (1) scope — "Shared payout engine + শুধু Investor side" (build the shared engine, Investor side only; Reseller commission deferred); (2) reseller commission rate — flat wholesale rate per product, reseller's commission = their selling price − (wholesale rate + other named costs) — recorded in the plan doc for when Reseller phase starts, not used in this round's code; (3) reseller commission hold period — 3 days (also recorded, not used yet); (4) payout batch approval — a single Super Admin, no maker-checker.

What was built (Investor/Channel-Partner side only — Reseller commission is still just the plan doc):

- **4 new migrations**: `company_payout_settings` (one row per company — `disbursing_bank_details` encrypted:array, `beftn_integration_mode` locked to `file_export_manual` for now, `beftn_api_credentials` reserved/unused), `payout_batches`, `payout_items` (polymorphic `payable`), and a retrofit adding `recipient_routing_number` to `settlement_payouts` plus a full `recipient_name`/`recipient_bank_name`/`recipient_branch`/`recipient_routing_number`/`recipient_account_number` set to `channel_partner_payouts` (which had no recipient/bank fields at all before — there was previously no way to record where a channel partner's share should go).
- **3 new models**: `CompanyPayoutSetting`, `PayoutBatch` (`hasMany` `PayoutItem`), `PayoutItem` (`morphTo` `payable`). `SettlementPayout`/`ChannelPartnerPayout` gained a `payoutItems()` morphMany, `hasActivePayoutItem()`, and `hasCompleteBankDetails()`. `Company` gained `payoutSetting()`. `SettlementService::calculateAndSettle()` now fills the channel-partner payout's `recipient_name` from the partner investor's name (was previously left blank).
- **`App\Contracts\BeftnFileFormatter`** (interface) + **`App\Services\Payouts\GenericBeftnCsvFormatter`** (its only implementation — a plain CSV; no bank's exact file spec is confirmed, so this is hand-uploaded/re-keyed into the bank's own portal, per the plan's Phase 1 design). Bound in `AppServiceProvider`.
- **`App\Services\Payouts\PayoutBatchService`** — the batch state machine: `createBatchFromPendingInvestorPayouts()` (pulls every pending `SettlementPayout`/`ChannelPartnerPayout` with complete bank details and no existing active `PayoutItem`), `approve()`, `cancel()` (draft only — releases items back to the pool), `generateExportFile()` (approved → writes the CSV via `CompanyStorageService::putPrivate()`), `markSubmitted()`, `reconcileItem()` (paid → syncs the underlying payout to `paid` with the bank reference; failed → leaves it `pending` for correction). Status flow: `draft → approved → file_generated → submitted_to_bank → completed | partially_completed | failed`, plus `draft → cancelled`. The plan's separate "pending_approval" status was dropped — draft already means "not yet approved," so it added a click without a new state.
- **Filament (Investments cluster)**: new **Payout Batches** resource (`ListPayoutBatches`'s "Generate New Batch" header action; `ViewPayoutBatch`'s Approve/Cancel/Generate Export File/Mark Submitted actions; `ItemsRelationManager`'s per-item Reconcile action) and a new **Payout Bank Account** page (`CompanyPayoutSettings` — strictly super-admin-only, not the general `settings.manage` permission, since it holds the account real money is drawn from). `PayoutsRelationManager`/`ChannelPartnerPayoutsRelationManager` gained a routing-number field, a "Bank Details: Complete/Incomplete" badge, and hide their old one-by-one "Mark as Paid"/Edit actions whenever the payout already has an active batch item (prevents a double payment).
- **Route + controller**: `GET /admin/payout-batches/{batch}/export` (`payout-batches.export`) → `PayoutBatchFileDownloadController`, mirroring the existing `InvestorContractDownloadController` pattern (company-scoped, `investments.settle`-gated, streamed from private storage).
- **Permissions**: no new permission was added — every payout-batch action reuses the existing `investments.settle` permission (today held only by `super_admin`), which already matches the owner's "single Super Admin approval" answer.
- **`10_INVESTOR_RESELLER_AUTO_PAYOUT_PLAN.md`** — marked Phase 1 built with a summary of what was and wasn't included; the "Owner Decision লাগবে" checklist updated with the three answers above (BEFTN bank-connectivity mode is still open — the owner is talking to their bank).

Verification:

- `composer install` and `npm install && npm run build` were needed first — this container started with no `vendor/`/`public/build/` at all (pre-existing environment gap, unrelated to this change).
- New tests: `tests/Feature/PayoutBatchTest.php` (8 cases — bank-details eligibility filter, double-batch guard, the full draft→completed lifecycle, a failed reconcile leaving the payout pending, cancel-then-rebatch, and channel-partner inclusion) and `tests/Feature/PayoutBatchPagesSmokeTest.php` (2 cases — both new pages render for a super admin, including a real batch's view page; the settings page 403s for a non-super-admin). `MultiCompanyIsolationTest` updated with the 3 new models.
- `php artisan test tests/Feature/PayoutBatchTest.php tests/Feature/PayoutBatchPagesSmokeTest.php tests/Feature/InvestmentSettlementTest.php tests/Feature/MultiCompanyIsolationTest.php` — 33 passed.
- Full `php artisan test` — **1197 passed, 1 failed**. The failure (`ReleaseNotesTest > database related release notes are hidden from a non super admin viewer`) is pre-existing and unrelated: `git diff CHANGELOG.md` against this session's starting point shows no change to any already-published version section (only this session's own `[Unreleased]` additions), and that test reads whatever the current latest *published* CHANGELOG entry already on this branch is — nothing this session touched.
- `vendor/bin/pint --dirty` — clean (auto-fixed import ordering/unused imports on 5 files, all cosmetic).
- `npm run build` — clean (no frontend asset changes beyond one new plain Filament-panel blade view, which needs no separate build step of its own).

Commit status: Committed + pushed to `origin/claude/auto-profit-payment-system-9acb0u` together with the Phase 2 entry above, in one commit (owner approved: "কমিট এবং পুশ কর, রিলিজ ভার্শন লেটেস্ট github ripo চেক করে লিখবে। এরপর কমিট এবং পুশ করবে শুধু তোমার এই সেশনের কাজগুলোই।").
