...
- Current published release is **v2.1.0** (2026-08-18), covering the Noor Solar Energy storefront, commerce/CRM/courier enhancements, compact admin UI refinements, and app-wide monetary display formatting; the configured version, release type, date, and latest `CHANGELOG.md` entry are covered by `ReleaseNotesTest` so they cannot drift apart.
