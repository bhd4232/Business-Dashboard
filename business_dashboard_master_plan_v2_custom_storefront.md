...
> **v6 আপডেট (Standalone ফাইল cross-reference + Part 13 Tasneem/Garments Machinery):** Part 11.2 (Lead/CRM) ও Part 11.3 (Investor)-এ এখন সরাসরি `02_LEAD_CRM_MODULE_PLAN.md` ও `01_INVESTOR_MUDARABAH_MODULE_PLAN.md` ফাইলের রেফারেন্স যুক্ত হয়েছে — এই দুটো standalone ফাইল Claude Code-কে সরাসরি দেওয়ার জন্য প্রস্তুত এবং সম্পূর্ণ আপডেটেড current-status snapshot সহ। নতুন **Part 13** যুক্ত হয়েছে Garments Machinery (Tasneem Knit Industry)-র জন্য — ব্যবহারকারী নিশ্চিত করেছেন যে WordPress redesign (আলাদা ডেভেলপারের প্ল্যান, এই Laravel কোডবেসের সম্পূর্ণ বাইরে) এবং আমাদের Laravel storefront-এ নতুন `quote_request` mode — দুটোই সমান্তরালে চলবে, ঠিক Part 12 (WooCommerce Migration)-এর মতোই "সমান্তরাল, ভবিষ্যতে migrate করা যাবে" মডেলে। Machine Request ফর্ম সরাসরি বিদ্যমান Lead/CRM module ব্যবহার করবে — কোনো নতুন model/সিস্টেম তৈরি হবে না।
...
## 3.8 External Global Courier Fraud Check (নতুন — Live Lookup by Phone) — ✅ বাস্তবায়িত (2026-07-07)

> **আপডেট:** `shahariar-ahmad/courier-fraud-checker-bd` প্যাকেজ ব্যবহার করে বাস্তবায়ন সম্পন্ন (`app/Services/ExternalCourierFraudService.php`, `app/Jobs/CheckExternalCourierFraudJob.php`)। Pathao-র কোনো অফিসিয়াল পাবলিক fraud-check API না থাকায় এই প্যাকেজ Pathao/Steadfast/RedX-এর merchant panel-এ সরাসরি লগইন করে ডেটা আনে — অফিসিয়াল API-র চেয়ে কম নির্ভরযোগ্য, ভবিষ্যতে কুরিয়ার ওয়েবসাইট পরিবর্তন হলে ভাঙতে পারে। বিস্তারিত: `CHANGELOG.md` [1.7.0] এবং `UPDATE_NOTES.md`।

> **প্রেক্ষাপট:** ব্যবহারকারী "Zayvik" নামে একটা ফেসবুক পোস্টে দেখা সার্ভিসের কথা উল্লেখ করেছিলেন — যাচাই করে দেখা গেছে এই নামে কোনো নির্দিষ্ট courier-fraud API খুঁজে পাওয়া যায়নি (এই নামে একটা video-editor hiring পোস্ট ও একটা microgreens ব্যবসা পাওয়া গেছে, দুটোই অসম্পর্কিত)। তাই এখানে **নাম-নির্দিষ্ট না রেখে "External Courier Fraud API"** হিসেবে ডিজাইন করা হলো, যাতে ভবিষ্যতে যেকোনো বাস্তব সার্ভিস (`courier-fraud-checker-bd` Laravel প্যাকেজ, `FraudBD` API, বা অন্য কোনো প্রতিষ্ঠিত বাংলাদেশি সার্ভিস) সহজে বসানো যায়।

### 3.8.1 মূল ধারণা — দুই জায়গায় ভিন্ন আচরণ

```txt
Admin Dashboard (Filament Order Create) — LIVE ও VISIBLE:
  Staff/owner ফোন নম্বর টাইপ করলে সাথে সাথে external API call হয় এবং
  ফলাফল (প্রতি কুরিয়ারে success/cancel/total + overall ratio) সরাসরি
  ফর্মে দেখানো হয়। এটা internal staff-এর সিদ্ধান্ত নেওয়ার টুল।

Storefront Checkout — SILENT ও HIDDEN:
  কাস্টমার নিজের ফোন নম্বর দিলে ব্যাকগ্রাউন্ডে একই external API call হয়,
  কিন্তু কাস্টমার কিছুই দেখেন না। ফলাফল সরাসরি বিদ্যমান CustomerRiskService-এর
  scoring pipeline-এ input হিসেবে যোগ হয় (Part 3.4-এর existing rule-based
  scoring-এর একটা নতুন factor)।

⚠️ এই দুটো আচরণ ব্যবহারকারীর স্পষ্ট সিদ্ধান্ত অনুযায়ী — storefront-এ কাস্টমার
নিজের রিস্ক ডেটা কখনো দেখবে না, শুধু admin/staff দেখবে।
```
...
মূল মেথড: checkByPhone(string $phone): array

দায়িত্ব:
- Cache প্রথমে চেক করা (phone নম্বর key দিয়ে, ২৪ ঘণ্টা TTL) — একই নম্বর
  বারবার external API call করা এড়াতে (cost + rate-limit সুরক্ষা)
- Cache miss হলে external API (courier-fraud-checker-bd প্যাকেজ বা সমতুল্য
  API) কল করা
- Response normalize করে standard array রিটার্ন করা:
  ['pathao' => ['success'=>5,'cancel'=>2,'total'=>7],
   'steadfast' => [...], 'overall_success_ratio' => 71.4]
- API timeout/failure হলে exception throw না করে null/empty array
  রিটার্ন করা — কখনোই Order creation বা checkout ব্লক করবে না
- প্রতিটা call CustomerRiskEvent হিসেবে log হওয়া (audit trail-এর জন্য,
  বিদ্যমান customer_risk_events টেবিল পুনঃব্যবহার করে, নতুন টেবিল না)
```

### 3.8.3 Admin Dashboard Implementation (Filament)

```txt
OrderForm-এর customer phone ফিল্ডে ->live(onBlur: true) যুক্ত হবে
(Filament 4-এর নেটিভ ফিচার, কোনো কাস্টম JS লাগবে না)

->afterStateUpdated() হুকে ExternalCourierFraudService::checkByPhone()
কল হবে, ফলাফল ফর্মের নিচে read-only Placeholder/Card component-এ:

┌─────────────────────────────────────┐
│ 📊 Courier History — 01712345678       │
│ Pathao:    ৫ Success / ২ Cancel (৭১%)  │
│ Steadfast: ৩ Success / ১ Cancel (৭৫%)  │
│ Overall Success Ratio: ৭৩% — 🟡 Medium│
└─────────────────────────────────────┘

API call চলাকালীন loading state (spinner) দেখানো; timeout হলে
"তথ্য পাওয়া যায়নি, ম্যানুয়ালি যাচাই করুন" বার্তা দেখানো।
```

### 3.8.4 Storefront Checkout Implementation (Silent)

```txt
CheckoutController-এ Order confirm হওয়ার পরে, বিদ্যমান Part 3 risk-check
pipeline-এ একটা নতুন ধাপ যোগ হবে:

Order confirmed → CustomerRiskService::evaluateOrder($order)
  → (বিদ্যমান) নিজস্ব company courier history চেক
  → (নতুন) ExternalCourierFraudService::checkByPhone() — queued job
    হিসেবে চালানো (synchronous না, যাতে external API slow/down থাকলে
    checkout experience-এ প্রভাব না পড়ে)
  → ফলাফল risk score calculation-এ একটা weighted factor হিসেবে যোগ হয়
  → সব মিলিয়ে final risk_level নির্ধারণ, বিদ্যমান badge/approval flow
    (Part 3.6) অপরিবর্তিতভাবে কাজ করবে

কাস্টমার এই প্রক্রিয়ার কিছুই দেখেন না — checkout স্বাভাবিকভাবে সম্পন্ন হয়।
```

### 3.8.5 গুরুত্বপূর্ণ বাস্তবায়ন সতর্কতা

```txt
[ ] Storefront checkout-এ external API call অবশ্যই queued job (async)
    হতে হবে, synchronous request-response cycle-এ না — নাহলে external
    API ধীর/ডাউন থাকলে কাস্টমারের checkout ব্যর্থ/ধীর হয়ে যাবে
[ ] Admin dashboard live call-এর জন্য cache TTL (২৪ ঘণ্টা) বাধ্যতামূলক —
    staff একই নম্বর বারবার টাইপ/এডিট করলে repeated cost এড়াতে
[ ] External API-র pricing/rate-limit model production-এ যাওয়ার আগে
    যাচাই করা প্রয়োজন (per-call charge হতে পারে) — বাজেট বিবেচনা করতে হবে
[ ] বিদ্যমান CustomerRiskService-এর সাথে duplicate/সাংঘর্ষিক কোনো নতুন
    scoring system তৈরি করা যাবে না — এই external check বিদ্যমান
    pipeline-এর একটা নতুন input factor মাত্র, আলাদা সিদ্ধান্ত-গ্রহণ
    ব্যবস্থা না
[ ] কোন specific external API/প্যাকেজ ব্যবহার করা হবে তা এখনো চূড়ান্ত
    সিদ্ধান্ত না — বাস্তবায়নের আগে ব্যবহারকারীর সাথে নিশ্চিত করে নিতে হবে
    (`courier-fraud-checker-bd` Composer প্যাকেজ সবচেয়ে সহজ ইন্টিগ্রেশন
    দেয় যেহেতু এটা native Laravel প্যাকেজ)
```
...
> **📄 বিস্তারিত implementation ফাইল:** `02_LEAD_CRM_MODULE_PLAN.md` — এই ফাইলে সম্পূর্ণ স্টেপ-বাই-স্টেপ migration, model, service, Filament resource, ও টেস্ট কোড আছে, Claude Code-কে সরাসরি দেওয়ার জন্য প্রস্তুত। নিচের অংশ শুধু সংক্ষিপ্ত সারাংশ ও master plan-এর মধ্যে sequencing বোঝানোর জন্য।
...
> **📄 বিস্তারিত implementation ফাইল:** `01_INVESTOR_MUDARABAH_MODULE_PLAN.md` — এই ফাইলে সম্পূর্ণ স্টেপ-বাই-স্টেপ migration, model, settlement calculation logic, Filament resource, ও টেস্ট কোড আছে, Claude Code-কে সরাসরি দেওয়ার জন্য প্রস্তুত। নিচের অংশ শুধু সংক্ষিপ্ত সারাংশ ও master plan-এর মধ্যে sequencing বোঝানোর জন্য।
...
# Part 13: Company-Specific Storefront Mode — Garments Machinery (Tasneem Knit Industry)

## 13.1 প্রেক্ষাপট ও ব্যবসায়িক সিদ্ধান্ত

Tasneem Knit Industry (`tasneemknitindustry.com`) — Garments Machinery company — একটা সম্পূর্ণ ভিন্ন ব্যবসায়িক মডেল: ৳৬,৮০,০০০–৳১০,৫০,০০০ মূল্যের শিল্প নিটিং মেশিন Taiwan/China থেকে import করে বিক্রি করা হয়, stock অনিশ্চিত থাকায় ব্যবসাটা আসলে **"machine sourcing service"** — কাস্টমার প্রয়োজন বর্ণনা করেন, কোম্পানি সেটা খুঁজে এনে দেয়। এখানে সাধারণ cart/checkout মডেল ভুল (একটা ডেভেলপারের দেওয়া WordPress redesign প্ল্যানেও এটাই মূল সমস্যা হিসেবে চিহ্নিত হয়েছে)।

> **✅ ব্যবহারকারীর সিদ্ধান্ত (দুটোই সমান্তরালে চলবে):**
> ১. **WordPress redesign** (আলাদা ডেভেলপারের PDF প্ল্যান অনুযায়ী) — এটা এখনকার জন্য বাস্তবায়িত হবে, `tasneemknitindustry.com`-এ এখনই থাকা WordPress+WooCommerce সাইটে। এই কাজ **আমাদের Laravel ERP/Master Plan-এর সম্পূর্ণ বাইরে** — Claude Code বা কোনো এজেন্ট এই Laravel কোডবেসে WordPress-সম্পর্কিত কোনো কাজ করবে না।
> ২. **আমাদের Laravel storefront-এও** Garments Machinery company-র জন্য সাপোর্ট তৈরি হবে, **`quote_request` storefront mode** হিসেবে — এটা এখনই urgent না, বরং **Part 12 (WooCommerce Data Migration)-এর মতো একই ধরনের "সমান্তরাল, ভবিষ্যতে migrate করা যাবে" মডেল**। যেদিন owner সিদ্ধান্ত নেবেন WordPress থেকে সরে আসার, সেদিন এই mode ব্যবহার করে migrate করা যাবে।

**এজেন্টের জন্য নির্দেশ:** এই Part 13-এর কোনো কাজ Part 4 (Storefront Foundation)-এর মূল `catalog_cart` mode সম্পন্ন হওয়ার আগে শুরু করবেন না — এটা storefront foundation-এর একটা extension, ভিত্তি না।

## 13.2 নতুন Storefront Mode — Design

```txt
companies.settings JSON-এ নতুন key:
{
  "storefront_mode": "catalog_cart" | "quote_request"
}

catalog_cart  — Solar/Gadget/Gift company-র বর্তমান ডিজাইন (tiered pricing,
                MOQ, cart, checkout) — অপরিবর্তিত
quote_request — Garments Machinery-র জন্য নতুন mode:
                - কোনো cart/checkout নেই
                - প্রতিটা product page-এ "Request Quote" + WhatsApp বাটন
                - Machine specs টেবিল (brand, origin, diameter, gauge,
                  feeders, condition — নতুন/ব্যবহৃত)
                - "Availability: In Stock / By Order" status badge
                - Indicative price দেখানো হয়, কিন্তু নোট সহ: "প্রকৃত মূল্য
                  availability ও exchange rate-এর উপর নির্ভরশীল — কোটেশনের
                  জন্য যোগাযোগ করুন"
```

## 13.3 Machine Request Form → Lead/CRM Integration (নতুন কিছু বানাতে হবে না)

> **গুরুত্বপূর্ণ:** এই "Machine Request" ফর্ম **হুবহু Lead/CRM module (Part 11.2, `02_LEAD_CRM_MODULE_PLAN.md`)-এর কাজ।** আলাদা কোনো নতুন form-submission সিস্টেম বা email-notification সিস্টেম তৈরি করা হবে না।

```txt
Flow:
কাস্টমার product page-এ "Request Quote" ক্লিক করেন
→ ফর্ম (name, company/factory, phone, WhatsApp, email, machine type,
  preferred brand, condition, specs, quantity, budget range, timeline, note)
  pre-filled machine name সহ খোলে
→ Submit হলে সরাসরি Lead তৈরি হয় (Part 11.2-এর Lead মডেল, company_id =
  Garments Machinery, source = website)
→ owner Filament-এর LeadResource-এ এই Lead দেখেন, review করেন
→ Quotation তৈরি করে পাঠান (WhatsApp শেয়ারযোগ্য লিংক — Part 11.2-এ
  ইতিমধ্যে ডিজাইন করা)
→ Accepted হলে Order-এ convert (বিদ্যমান flow অপরিবর্তিত)

কোনো নতুন "MachineRequest" model/টেবিল তৈরি হবে না — এটা শুধু বিদ্যমান
Lead মডেলের একটা নতুন UI entry point (product-page-embedded form)।
```

## 13.4 Product Model Extension (শুধু Garments Machinery-র জন্য প্রাসঙ্গিক)

```txt
products টেবিলে (বা company-নির্দিষ্ট হলে একটা আলাদা product_specs JSON
কলাম):
  machine_brand, machine_origin, diameter, gauge, feeders,
  condition (new/used/either), availability_status (in_stock/by_order)

⚠️ এই ফিল্ডগুলো শুধু Garments Machinery company-র product-এ প্রাসঙ্গিক।
Filament ProductResource-এ conditional visibility ব্যবহার করুন
(company-র storefront_mode অনুযায়ী এই ফিল্ড group দেখানো/লুকানো) — অন্য
company-র product form-এ এই ফিল্ড দেখিয়ে বিভ্রান্তি তৈরি করবেন না।
```

## 13.5 Trust Bar ও How-It-Works (Top-Class Pattern-এর সাথে যুক্ত)

WordPress প্ল্যানের এই দুটো উপাদান আমাদের Part 4.6-এর "Top-Class Reference Pattern"-এর সাথে সরাসরি মেলে — নতুন কিছু ডিজাইন করার দরকার নেই, বিদ্যমান প্যাটার্ন ব্যবহার করুন:

```txt
Trust Bar (SGS/BV/ITS inspection, CFR Chattogram, BIN/IRC licensed)
  → storefront_settings-এ একটা "trust_badges" JSON array ফিল্ড, Filament
    থেকে admin-editable (Part 4.9-এর Admin-Storefront Sync নিয়ম অনুযায়ী —
    hardcoded করা যাবে না)

How Buying Works (৪-৫ ধাপ: inquiry → sourcing → quotation → L/C payment
→ inspection → CFR shipping → installation)
  → Part 4.6-এর "কিভাবে অর্ডার করবেন" explainer প্যাটার্নের extended
    version, StorefrontPageResource দিয়ে ম্যানেজড
```

## 13.6 যা এই Part-এ স্পষ্টভাবে বাদ

```txt
WordPress সাইটের কোনো কোড/থিম/প্লাগিন এই Laravel কোডবেসে স্পর্শ করা হবে না —
  সম্পূর্ণ আলাদা সিস্টেম, আলাদা ডেভেলপার কাজ করছেন।
L/C payment gateway integration — এখনো স্কোপে নেই, ম্যানুয়াল প্রক্রিয়া থাকবে।
WooCommerce থেকে ডেটা migration — এটা Part 12-এর সাধারণ নিয়মেই কভার হবে,
  Garments Machinery company-র জন্য আলাদা কিছু লাগবে না।
```

## 13.7 Build Order-এ অবস্থান

```txt
Part 4 (Storefront Foundation, catalog_cart mode) ✅ সম্পন্ন
        ↓
Part 11.2 (Lead/CRM Module) — সম্পন্ন হতে হবে, কারণ quote_request mode
        এর উপর নির্ভরশীল
        ↓
Part 13 (এই Part) — quote_request mode + machine specs fields + trust bar
        ↓
(ঐচ্ছিক, ব্যবহারকারীর সিদ্ধান্ত অনুযায়ী ভবিষ্যতে) WordPress থেকে সম্পূর্ণ
        migrate করে Tasneem-ও আমাদের Laravel storefront-এ আনা
```

**এজেন্টের জন্য সংক্ষিপ্ত নির্দেশ:** এই Part 13 এখনই urgent কাজ না। WordPress redesign স্বাধীনভাবে এগিয়ে যাবে। এই Part শুধু তখনই শুরু করুন যখন ব্যবহারকারী স্পষ্টভাবে বলবেন Lead/CRM module সম্পন্ন হয়ে গেছে এবং এখন Garments Machinery-র জন্য Laravel storefront mode বানানোর সময় হয়েছে।
