...
**Status:** Done, per the 2026-09-07 gap analysis's own three-sprint plan. The calculation core shipped first; a 2026-09-07 audit against the client's real signed documents then found the contract/legal fields, document management, per-investor reporting, loss handling, and a settlement-correction path all missing. See `01_INVESTOR_MODULE_v3_GAP_ANALYSIS.md` for the full gap list and progress. Sprint 1 (P1.1–P1.7 — contract fields, company gap-fill contribution, corrected annualized return, document attachments, printable investor report + register, settlement draft/confirm/void, loss handling), Sprint 2 (P2.1–P2.5 — channel-partner modeling, code-audit bug fixes, an optional investment window, notice-period + withdraw/reinvest tracking, and ledger/Voucher integration), and Sprint 3 (P3 — PII encryption, gross profit on the settlement infolist, inline cheque number, UI-level test coverage, and a real bug fix found during that audit) are all complete in the working tree.
...
- **(Sprint 3, 2026-09-13)** `nid_number`/`nominee_nid_or_passport`/`guarantor_nid` are now encrypted at rest (Eloquent `encrypted` cast, same pattern as ConversationChannel tokens); gross profit shown on the settlement infolist (previously only on the printable report); investment cheque number shown inline on the project's investments table; UI-level test coverage added for the settle action's form, both mark-paid flows, a permission-denied path, and security-instrument create/upload; `AdminNavigationClustersTest` now covers the Investments cluster.
- **(Sprint 3)** Bug fix found during the P3 audit: `SecurityInstrumentsRelationManager`/`WitnessesRelationManager` (on `InvestmentRecordResource`), `DocumentsRelationManager` (also on `ProjectSettlementResource`), and `PayoutsRelationManager`'s `EditAction` (also on `ProjectSettlementResource`) were permanently read-only in the admin UI — those resources have no Edit page, so Filament's "relation managers are read-only on View pages" default never had a writable page to fall back to. Security instruments, settlement-level documents, and a mistyped payout recipient/bank detail could not actually be created or corrected before this fix.
