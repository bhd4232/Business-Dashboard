...
- **Investor & Channel-Partner payouts can now be grouped into a BEFTN bank payout batch, approved, and exported as a bank-ready file — Investments → Payout Batches.** Where a settlement's investor/channel-partner payout previously had to be marked "paid" one at a time by hand, **Payout Batches → Generate New Batch** now pulls in every pending payout across the whole company that has complete bank details (bank name, branch, routing number, account number) into one batch. A batch always goes through the same four deliberate steps — nothing is ever submitted to a bank automatically: **Approve** (a separate, explicit step before any file is produced), **Generate Export File** (a downloadable CSV formatted for hand-upload into the bank's own corporate/bulk-payment portal), **Mark Submitted to Bank** (optionally records the bank's own batch/UTR reference), then **Reconcile** each item against the bank's confirmation — reconciling an item "Paid" marks the underlying investor/channel-partner payout paid with the bank's transaction reference; "Failed" leaves it pending so it can be corrected and re-batched. A payout already sitting in an active batch has its old one-by-one "Mark as Paid" button hidden so it can't be paid twice, and a payout missing any bank detail is shown a plain "Incomplete" badge instead of being silently skipped. The company's own disbursing bank account is configured once on the new **Payout Bank Account** page (super-admin only, encrypted at rest).
  Channel-Partner payouts gained their own recipient bank fields (name, bank, branch, routing number, account number) for the first time — previously there was nowhere to record where a channel partner's 10% share should be sent, only that it had been paid.

  **Technical Notes:** new `company_payout_settings` (one row per company, `disbursing_bank_details` encrypted:array), `payout_batches`, and `payout_items` (polymorphic `payable` — `SettlementPayout` or `ChannelPartnerPayout` today) tables; `recipient_routing_number` added to `settlement_payouts`, and a full `recipient_name`/`recipient_bank_name`/`recipient_branch`/`recipient_routing_number`/`recipient_account_number` set added to `channel_partner_payouts` (it had none before). New `App\Services\Payouts\PayoutBatchService` runs the batch state machine (`draft → approved → file_generated → submitted_to_bank → completed | partially_completed | failed`, plus `draft → cancelled`) and a `App\Contracts\BeftnFileFormatter` interface with one `GenericBeftnCsvFormatter` implementation (no bank's exact file spec is confirmed yet — see `10_INVESTOR_RESELLER_AUTO_PAYOUT_PLAN.md` Phase 2 for the swap-in path once one is). Reuses the existing `investments.settle` permission for every payout-batch action; no new permission was added. `MultiCompanyIsolationTest` covers the three new models; `PayoutBatchTest` covers the full batch lifecycle, the double-batch guard, and the bank-details eligibility filter.

- **Resellers now earn a tracked commission on every delivered order, paid out through the same batch engine — Resellers → a reseller's Store Products, Commissions, and Commission Payout sections.** Staff set a wholesale rate per reseller-per-product (**Store Products → Set Wholesale Rate**); the moment that reseller's order is marked delivered, a commission record is created for (the order's actual sale price − that wholesale rate), summed across items — a product with no wholesale rate set yet contributes nothing rather than guessing one. It sits in a 3-day (configurable) hold so a return can still cancel it, then a daily job promotes it to payable automatically. Staff can itemize deductions (packaging, return handling, courier/delivery charge, or anything else) on the **Commissions** tab any time before it's paid, which reduce the final commission. The reseller picks how they're paid — bank account or bKash/Nagad/Rocket — on their **Commission Payout** section; **Payout Batches → Generate New Batch** now offers a Reseller-commissions source with a method picker, running through the exact same Approve → Generate Export File → Mark Submitted → Reconcile flow as Investor payouts (a plain CSV for the mobile-wallet methods too, since no MFS provider's disbursement API is connected yet — reconciled the same manual way). A commission already sent is never auto-reversed by a later return; that's flagged in the audit log for manual handling until a recovery rule is confirmed. Reseller catalog curation and pricing stay otherwise unchanged — a reseller's storefront still shows the same price everyone else sees; setting their own retail price is a deferred follow-up, not part of this change.

  **Technical Notes:** new `reseller_commissions` table (polymorphic `payable` alongside `SettlementPayout`/`ChannelPartnerPayout` in the shared `PayoutBatch`/`PayoutItem` engine); `reseller_products.wholesale_rate`, `customers.reseller_payout_method`/`reseller_payout_details` (encrypted:array), `company_payout_settings.reseller_commission_hold_days` (default 3), `payout_items.recipient_mfs_number`. New `App\Observers\ResellerCommissionObserver` (mirrors `OrderNotificationObserver`'s `status`+`delivery_status` check — every path that marks an order delivered converges on `OrderStatusWorkflowService::transition()` setting both together) and `App\Services\Reseller\ResellerCommissionService` (create/promote/reverse); new `reseller-commissions:promote` scheduled command (daily 01:00, per-company). `PayoutBatchService` gained `createBatchFromPendingResellerCommissions()` (kept separate from the Investor batch call — the two are never combined into one batch this round) and a `PayoutFormatterResolver` to pick between the existing BEFTN CSV formatter and a new `GenericMfsListFormatter` for `mfs_*` methods. No new permission — gated by the same `Customer`-update permission the Reseller resource already uses. `ResellerCommissionTest` and `ResellerCommissionPayoutBatchTest` cover the calculation, hold/promote/reverse lifecycle, and the batch/reconcile flow for both payout rails.

- **Planning only — Investor profit & Reseller commission auto bank/MFS payout.** Added `10_INVESTOR_RESELLER_AUTO_PAYOUT_PLAN.md`: a design for a shared payout engine (`PayoutBatch`/`PayoutItem` + a `BeftnFileFormatter` adapter interface, plus a new `ResellerCommission` model with a configurable delivery-then-hold-window flow) that would let Investor settlement payouts and Reseller order-commission payouts be calculated, batched, and exported for bank (BEFTN, for one-debit/many-credit transfers — not NPSB, which is single-transaction/real-time) or MFS (bKash/Nagad/Rocket) disbursement, with a mandatory human-approval step before anything reaches a bank/MFS submission. Both halves of this plan are now built — see the two entries above.
